...
locals {
  ...
  variable_subsitition_step = [{
    ...
    args = [
      "-c",
      "curl -s https://raw.githubusercontent.com/BHI-Residential/gcp-cloudbuild-substitution-variables/v1.0.0/gcsvh.sh | bash"
    ]
  ...
  vpc_access_connector_step = {
    ...
    args = [
      "-c",
      "curl -s https://raw.githubusercontent.com/BHI-Residential/gcp-app-engine-flexible-configure-vpc/v1.0.0/gaefcv.sh | bash -s ${var.appengine_vpc_access} ${var.workspace}-vpc ${var.workspace}-${var.appengine_region}-private-subnet"
    ]
  }
